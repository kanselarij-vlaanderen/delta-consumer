...
> [!WARNING]  
> This repository is a fork of [lblod/delta-consumer](https://github.com/lblod/delta-consumer)!
> If you need the delta consumer, you probably want to use that one.
> This repository contains a WIP rewrite of the configurable consumer API that replaces the triples-dispatching interface.
> The rewrite is more of an experiment, and it's not certain if it will ever be merged.
> The fork exists so that we can use it inside Kaleidos with proper versioning.
...
By default, the delta-consumer will use the fetched information and store in their respective graphs as generated by the producer. To maintain a single ingest graph, set the `TARGET_GRAPH` environment variable.

However, custom ingestion rules are perfectly possible. Read along if you want to know how you can achieve this.
'Dispatching' is the term we will use when moving triples to the correct place.

## Tutorials
### Add the service to a stack, with default behaviour.
The default behaviour fetches the information from the producer and stores the triples in the graphs set by producer.
...
To force all triples to be ingested in a single graph add the following environment variable:

``` yaml
consumer:
  environment:
    TARGET_GRAPH: 'http://uri/of/the/graph/to/ingest/the/information'
```
...
- `TARGET_GRAPH (default: unset)`: graph in which to write the triples ingested by the delta sync
