# delta-consumer
Extendable consumer service to sync data from external sources based on diff files generated by a producer
...
Mounted in `./config/changeset-transformer.js`
...
###### Triple transformer
Mounted in `./config/transformer.js`
...
###### Dispatcher
Mounted in `./config/dispatch.js`

Exports an async function that is called on each diff file (after transformations). Determines how triples are dispatched in the triplestore at the consumer side. May also add additional enrichments/behaviours like for example downloading share files.
