import { query, update, sparqlEscapeDateTime, sparqlEscapeString, sparqlEscapeUri, uuid } from 'mu';
import { JOB_TYPE, JOB_URI_PREFIX, PREFIXES, STATUS_SCHEDULED, STATUS_FAILED, STATUS_BUSY } from './constants';
...
export async function createJob(jobsGraph,
  ...
  await update(createJobQuery, { sudo: true });
...
export async function loadJob(subject){
  ...
  const job = parseResult(await query(queryJob, { sudo: true }))[0];
  if(!job) return null;
  ...
  const tasks = parseResult(await query(queryTasks, { sudo: true })).map(row => row.task);
  job.tasks = tasks;
...
export async function getJobs(jobOperationUri, statusFilterIn = [], statusFilterNotIn = []){
  ...
  `;
  const result = await query(queryIsActive, { sudo: true });
...
export async function cleanupJob(job) {
  ...
  `;
  await update(cleanupQuery, { sudo: true });
}

export async function setDanglingJobsStatusToFailed() {
  console.log("Cleaning up dangling jobs.");
  await update(`
    ${PREFIXES}
